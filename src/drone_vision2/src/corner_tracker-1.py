...
""" corner_tracker.py - Version 1.0 2016-02-23

    Based on the R. Patrick Goebel's lk_tracker.py demo code

    Tracks the goodFeaturetoTrack feature closest to the center of the image in a video stream
...
class CornerTracker(CornerDetector):
    def __init__(self, node_name):
        super(CornerTracker, self).__init__(node_name)
    ...
    def track_feature_point(self, grey, prev_grey):
        ...
        p0 = self.feature_point.reshape(-1, 1 ,2)
        ...
        try:
            ...
            for (x, y), good_flag in zip(p1.reshape(-1, 2), good):
                if not good_flag:
                    continue
                new_feature_point = np.array((x, y))
            ...
            world_cordinate = np.dot(np.linalg.pinv(self.projectionMatrix), np.array([self.feature_point[0],self.feature_point[1], 1]))
            world_cordinate = world_cordinate / world_cordinate[2]
            world_cordinate = np.array((world_cordinate[0], world_cordinate[1]))

            # Provide self.tracked_point to publish_poi to be published on the /poi topic
            self.tracked_point = world_cordinate
        except:
            self.tracked_point = None
...
if __name__ == '__main__':
    try:
        node_name = "corner_tracker"
        CornerTracker(node_name)
        rospy.spin()
    except KeyboardInterrupt:
        print "Shutting down Corner Tracking node."
